...
url = u"https://api.metalab.csun.edu/curriculum/api/2.0/terms/" + \
    sys.argv[1] + "-" + sys.argv[2] + "/classes/" + sys.argv[3].lower()
...
for course in data['classes']:
    if (current_class != course["title"]):
        ...
        blob_list.append(course['subject'] + ' ' +
                         course['catalog_number'] + ' ' + course["title"])
        blob_list.append("")
        blob_list.append(
    ...
    if (len(course["meetings"]) > 0):
        ...
        section_string.append("\t\t    " + (course["meetings"][0]["start_time"])[0:2] + ":" + (course["meetings"][0]["start_time"])[
                              2:4] + " - " + (course["meetings"][0]["end_time"])[0:2] + ":" + (course["meetings"][0]["end_time"])[2:4])
